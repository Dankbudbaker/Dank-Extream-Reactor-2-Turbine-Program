...
function createButtons()
    --In Deutsch
    if lang == "de" then
        page:add("Hauptmenue", function() run("/dankturbinecontrol_1/start/menu.lua") end, 2, 22, 17, 22)
        --In Englisch
    elseif lang == "en" then
        page:add("Main Menu", function() run("/dankturbinecontrol_1/start/menu.lua") end, 2, 22, 17, 22)
    end
...
function getEnergy()
    local en = v.getEnergy("unknown")
    local enMax
    if v == r then enMax = "140000000"
    else
        enMax = v.getEnergyCapacity("unknown")
    end
