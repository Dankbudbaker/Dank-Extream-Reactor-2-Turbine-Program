...
function _G.saveOptionFile()
	...
	local file = fs.open("/dankturbinecontrol_1/config/options.txt","w")
    file.writeLine(list)
	file.close()
	print("Saved.")
end
...
function _G.checkUpdates()
	...
	downloadFile("https://raw.githubusercontent.com/Dankbudbaker/Dank-Extream-Reactor-2-Turbine-Program/"..currBranch.."/dankturbinecontrol_1/src/",currBranch..".ver")
		...
		while true do
			...
			if event == "key" then
				...
				if p1 == 36 or p1 == 21 then
					shell.run("/dankturbinecontrol_1/install/installer.lua update "..branch)
					out = true
					break
				end
...
function _G.initPeripherals()
	...
	for i = 1, #peripheralList do
		--Turbines
		if peripheral.getType(peripheralList[i]) == "BiggerReactors_Turbine_" then
			t[amountTurbines] = peripheral.wrap(peripheralList[i])
			_G.amountTurbines = amountTurbines + 1
			--Reactor
		elseif peripheral.getType(peripheralList[i]) == "BiggerReactors_Reactor_" then
		...
		elseif peripheral.getType(peripheralList[i]) == "monitor" then
			...
			local stat,err = pcall(function() tmp.getEnergy("unknown") end)
			if stat then
				_G.v = tmp
			end
		end
...
if mainMenu then
	...
	shell.completeProgram("/dankturbinecontrol_1/start/start.lua")
else
